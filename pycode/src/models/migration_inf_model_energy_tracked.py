"""Implementation of model with migration at cost infinite with energy tracked."""
...
def run_instance(
    ...
    v = np.array([[model.add_var(var_type=mip.CONTINUOUS, lb=0, ub=instance.C[k])
                   for k in K] for _ in T])
    s = np.array([[model.add_var(var_type=mip.CONTINUOUS, lb=0)
    ...
    model.objective = mip.minimize(
        ...
        mip.xsum(s[t, k] for t in T for k in K)
    )
    ...
    for k in K:
        ...
        model += z[t_min, k] + instance.e[t_min, k] + instance.p[k] == \
                 v[t_min, k] + g[t_min, k] + s[t_min, k]
    # (4)
    for k, t in itertools.product(K, T[1:]):
        model += z[t, k] + instance.e[t, k] + g[t - 1, k] == v[t, k] + g[t, k] + s[t, k]
    ...
    x_correct_shape = np.full((instance.time_slots, instance.n_ap, instance.n_facility),
                              x)
    result = Result.from_variables(x=x_correct_shape, g=g, v=v, z=z, make_copy=False)
    # Removes the sum of the 's' values from the objective function.
    extract_var_value = np.vectorize(lambda var: var.x)
    real_object_value = model.objective_value - extract_var_value(s).sum()
    stats = ResultStats(status=status,
                        time=timestamp_end - timestamp_start,
                        obj_value=real_object_value,
